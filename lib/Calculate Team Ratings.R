...
for(i in 1:length(teams)) {
  club_players[[i]] <- teamstats[teamstats$Club == teams[i], ]
}

club_players[[1]]

names(club_players) <- teams
...
attack_score <- attack_rating(club_best_players)
attack_score
...
midfield_score <- midfield_rating(club_best_players)
midfield_score
...
defense_score <- defense_rating(club_best_players)
defense_score
...
GK_score <- GK_rating(club_best_players)
GK_score
...
allposition_score <- allposition_rating(club_best_players)
allposition_score
...
team_ratings <- team_rating(club_best_players)
rownames(team_ratings) <- teams
team_ratings["Newcastle",]

write.csv(team_ratings, file = "clubs_rating.csv")
